# Sensor Node for DiashowDL (Rust)

This Rust binary implements the DiashowDL
...
On boards with limited RAM (e.g. NanoPi with 512 MB), restrict parallel
compilation to avoid running out of memory:
...
./target/release/diashow-sensor-node
```

## Cross-Compilation

To build on another machine for Raspberry Pi:

```bash
# Install cross-compilation tool
cargo install cross

# Build for 64-bit Pi (Pi 3/4/5 with 64-bit OS)
...
ExecStart=/home/pi/diashow-sensor/diashow-sensor-node
Restart=on-failure
RestartSec=5
