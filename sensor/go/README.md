# Sensor Node for DiashowDL (Go)

This Go binary implements the DiashowDL
...
go build -o diashow-sensor-node
...
```bash
go run main.go sensor_linux.go
...
GOOS=linux GOARCH=arm64 go build -o diashow-sensor-node

# Build for 32-bit Pi
GOOS=linux GOARCH=arm GOARM=7 go build -o diashow-sensor-node
```
